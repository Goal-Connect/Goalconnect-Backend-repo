...
const normalizeProfilePayload = (body) => ({
  ...
  primaryPosition: body.primaryPosition ?? body.primary_position ?? body.position,
...
const updateProfile = async (req, res) => {
  try {
    ...
    const academy = req.user.role === 'academy'
      ? await Academy.findOne({ user: req.user._id })
      : null;

    const isAdmin = req.user.role === 'admin';
    const isPlayerOwner = req.user.role === 'player' && player.user && player.user.toString() === req.user._id.toString();
    const isSameAcademy = academy && player.academy && player.academy.toString() === academy._id.toString();
    const isAcademyRep = req.user.role === 'academy' && req.user.isAcademyRep && isSameAcademy;
    ...
    if (restrictedFieldsRequested && !(isAdmin || isAcademyRep)) {
      return res.status(403).json({
        success: false,
        message: 'Only an admin or academy rep can update verification or disciplinary record fields',
      });
    }
    ...
    if (value.secondaryPosition !== undefined) player.secondaryPosition = value.secondaryPosition;
    if (value.preferredFoot !== undefined) player.strongFoot = value.preferredFoot;
    if (value.jerseyNumber !== undefined) player.jerseyNumber = value.jerseyNumber;
    if (value.clubHistory !== undefined) player.clubHistory = value.clubHistory;
    if (value.playingStyleTags !== undefined) player.playingStyleTags = value.playingStyleTags;
    if (value.technicalStrengths !== undefined) player.technicalStrengths = value.technicalStrengths;
    if (value.technicalWeaknesses !== undefined) player.technicalWeaknesses = value.technicalWeaknesses;
    if (value.availabilityStatus !== undefined) player.availabilityStatus = value.availabilityStatus;
    if (value.birthCertificateUrl !== undefined) player.birthCertificateUrl = value.birthCertificateUrl;
    if (value.passportUrl !== undefined) player.passportUrl = value.passportUrl;
    if (value.isAgeVerified !== undefined) player.isAgeVerified = value.isAgeVerified;
    ...
    if (value.disciplinaryRecord !== undefined) {
      player.disciplinaryRecord = {
        ...(player.disciplinaryRecord?.toObject ? player.disciplinaryRecord.toObject() : player.disciplinaryRecord || {}),
